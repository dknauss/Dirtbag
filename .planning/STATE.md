...
- **Browser coverage is green where it runs, but Playground boot reliability remains a release risk.** The 2026-08-23 `main` E2E run passed the main suite and six style legs; the Minimalist leg hit an SQLite `Database Error` before Playwright could run, and an isolated rerun failed the same way on all three boot attempts. The harness fix now waits for Playground's own ready signal before the readiness probes can race its SQLite seed writes, and pins six request workers because the CLI warns that its three-worker default on GitHub's four-CPU runners increases file-lock deadlock risk. The exact failing Minimalist blueprint then booted locally and passed all 28 applicable browser/style tests (4 self-switching tests intentionally skipped). Track the remaining reliability work in GitHub issues #99 and #128.
...
1. Confirm the six-worker Playground change makes the complete GitHub E2E matrix green.
...
- Per-style matrix in that run: six legs green; Minimalist did not reach Playwright because Playground returned an SQLite `Database Error`.
- Exact Minimalist blueprint with six workers: green locally on 2026-08-23; 28 passed, 4 intentionally skipped.
