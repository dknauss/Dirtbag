...
* The per-style CI matrix never actually applied the style variation, so every leg scanned the **default** style while reporting per-style coverage — making the `color-contrast` "gated across every style" claim vacuous. Two independent causes, both in `playground/apply-style.php`: core's `get_user_global_styles_post_id()` creates the post via `wp_insert_post( ..., 'tax_input' )`, which is only honoured when the current user can assign terms — with nobody logged in during a Playground boot step the post was created *without* its `wp_theme` term, so every later lookup (including the front end's) missed it, created another orphan, and fell back to `theme.json`; and core's `wp_filter_global_styles_post` kses pass stripped the variation's `settings.custom` tree on save, which is why `truckIconFilter` never applied. The applier now links the term explicitly, drops the kses filter for its own write, verifies the result is a post WordPress can resolve, and throws rather than exiting — a Playground `runPHP` step swallows STDOUT, STDERR and the exit code, so only an exception fails the boot.
* Gate the sweep on rendered output (`tests/assert-style-applied.mjs`), locally and in CI, so an unapplied variation fails loudly instead of silently re-scanning the default. `head-meta.spec.js` and `truck-icon.spec.js` are no longer skipped in CI.
