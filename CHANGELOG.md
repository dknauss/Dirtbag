...
* Style variations applied by `playground/apply-style.php` lost their whole `settings` tree. Core hooks `wp_filter_global_styles_post()` onto `content_save_pre` for anyone lacking `unfiltered_html`, and its `remove_insecure_properties()` pass drops the free-form `settings.custom` before it reaches the database — so `settings.custom.dirtbag.truckIconFilter` never survived, and the truck logo kept theme.json's `none` on every variation. The applier drops that one filter for its own write (the payload is the theme's own `styles/<slug>.json`) and now verifies both layers landed.
* The applier's guards are effective in CI at last. They reported failure via `fwrite( STDERR, … )` and `exit( 1 )`, but a Playground `runPHP` step swallows both — and `STDERR` is a CLI-SAPI constant that is undefined there. Failures now throw, the one mode that fails the boot instead of letting the sweep run against an unapplied variation.
* `tests/styles/head-meta.spec.js` and `tests/styles/truck-icon.spec.js` no longer skip in CI. Both skips blamed Playground for what were really the bugs above.
